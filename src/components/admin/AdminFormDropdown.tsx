...
interface AdminFormDropdownOption {
  value: string
  label: string
  disabled?: boolean
}

interface AdminFormDropdownProps {
  value: string
  onChange: (value: string) => void
  options: AdminFormDropdownOption[]
  placeholder?: string
  disabled?: boolean
  className?: string
  width?: string
...
const AdminFormDropdown = forwardRef<HTMLInputElement, AdminFormDropdownProps>(
  ({ 
    value,
    onChange,
    options,
    ...
    return (
      <div className={`relative ${width} ${className}`} ref={dropdownRef}>
        ...
        >
          ...
          <ChevronDownIcon 
            className={`h-5 w-5 text-white/70 transition-transform duration-200 flex-shrink-0 ${
...
AdminFormDropdown.displayName = 'AdminFormDropdown'
