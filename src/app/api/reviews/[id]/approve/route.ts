...
export async function POST(
  ...
  try {
    ...
    const updatedReview = await prisma.review.update({
      ...
      data: {
        status: 'APPROVED',
        comments: comments || 'Asset approved',
        updatedAt: new Date()
      }
    })
    ...
    await prisma.activity.create({
      data: {
        type: 'ASSET_APPROVED',
        description: `Approved asset "${review.asset.title}"`,
        userId: user!.id,
    ...
    await prisma.notification.create({
      data: {
        ...
        message: `Your asset "${review.asset.title}" has been approved`,
        metadata: {
          assetId: review.assetId,
          reviewId: review.id,
          status: 'APPROVED',
    ...
    return ApiErrors.SERVER_ERROR('Failed to approve review')
  }
}